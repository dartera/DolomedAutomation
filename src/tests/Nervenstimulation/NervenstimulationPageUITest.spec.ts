...
// Define language configs for the Nervenstimulation page
const nervenstimulationLanguages = [
  {
    url: '/nervenstimulation/',
    code: 'de',
    name: 'German (Default)',
    isDefault: true
  },
  {
    url: '/fr/nervenstimulation/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Nervenstimulation Page (Desktop)', () => {
  ...
  for (const lang of nervenstimulationLanguages) {
    test(`nervenstimulation-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'nervenstimulation-visual',
        {
...
test.describe('Visual Regression - Nervenstimulation Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of nervenstimulationLanguages) {
    test(`nervenstimulation-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'nervenstimulation-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
