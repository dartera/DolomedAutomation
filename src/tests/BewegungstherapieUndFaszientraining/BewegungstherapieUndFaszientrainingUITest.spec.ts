    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `BewegungstherapieUndFaszientraining_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `BewegungstherapieUndFaszientraining_Mobile_${lang}`,
                    'BewegungstherapieUndFaszientrainingPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for bewegungstherapie-und-faszientraining
    if (lang === 'de') return '/bewegungstherapie-und-faszientraining/';
    
    // All other languages use their language code in the URL
    return `/${lang}/bewegungstherapie-und-faszientraining/`;
} 