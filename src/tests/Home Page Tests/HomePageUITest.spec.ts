...
// Define language configs for the Home page
const homeLanguages = [
  {
    url: '/',
    code: 'de',
...
test.describe('Visual Regression - Home Page (Desktop)', () => {
  ...
  for (const lang of homeLanguages) {
    test(`home-visual-desktop-${lang.code}`, async () => {
...
test.describe('Visual Regression - Home Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of homeLanguages) {
    test(`home-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'home-visual-mobile',
        {
