...
// Define language configs for the PRP page
const prpLanguages = [
  {
    url: '/plasma-prp/',
    code: 'de',
...
test.describe('Visual Regression - PRP Page (Desktop)', () => {
  ...
  for (const lang of prpLanguages) {
    test(`prp-visual-desktop-${lang.code}`, async () => {
...
test.describe('Visual Regression - PRP Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of prpLanguages) {
    test(`prp-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'prp-visual-mobile',
        {
