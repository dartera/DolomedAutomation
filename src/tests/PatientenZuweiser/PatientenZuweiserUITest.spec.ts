    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `PatientenZuweiserPage_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `PatientenZuweiserPage_Mobile_${lang}`,
                    'PatientenZuweiserPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for patienten-zuweiser
    if (lang === 'de') return '/patienten-zuweiser/';
    
    // All other languages use their language code in the URL
    return `/${lang}/patienten-zuweiser/`;
} 