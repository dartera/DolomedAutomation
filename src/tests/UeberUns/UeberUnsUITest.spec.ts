...
// Define language configs for the Ueber Uns page
const ueberUnsLanguages = [
  {
    url: '/ueber-uns/',
    code: 'de',
    name: 'German (Default)',
...
test.describe('Visual Regression - Ueber Uns Page (Desktop)', () => {
  ...
  for (const lang of ueberUnsLanguages) {
    test(`ueber-uns-visual-desktop-${lang.code}`, async () => {
...
test.describe('Visual Regression - Ueber Uns Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of ueberUnsLanguages) {
    test(`ueber-uns-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'ueber-uns-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
