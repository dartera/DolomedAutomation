    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `Gefaesserkrankungen_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `Gefaesserkrankungen_Mobile_${lang}`,
                    'GefaesserkrankungenPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for gefaesserkrankungen
    if (lang === 'de') return '/gefaesserkrankungen/';
    
    // All other languages use their language code in the URL
    return `/${lang}/gefaesserkrankungen/`;
} 