...
// Define language configs for the Gefaesserkrankungen page
const gefaesserkrankungenLanguages = [
  {
    url: '/gefaesserkrankungen/',
    code: 'de',
    name: 'German (Default)',
    isDefault: true
  },
  {
    url: '/fr/gefaesserkrankungen/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Gefaesserkrankungen Page (Desktop)', () => {
  ...
  for (const lang of gefaesserkrankungenLanguages) {
    test(`gefaesserkrankungen-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'gefaesserkrankungen-visual',
        {
...
test.describe('Visual Regression - Gefaesserkrankungen Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of gefaesserkrankungenLanguages) {
    test(`gefaesserkrankungen-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'gefaesserkrankungen-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
