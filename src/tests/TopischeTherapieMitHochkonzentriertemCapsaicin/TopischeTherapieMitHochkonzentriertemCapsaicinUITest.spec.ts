    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `TopischeTherapieMitHochkonzentriertemCapsaicinPage_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `TopischeTherapieMitHochkonzentriertemCapsaicinPage_Mobile_${lang}`,
                    'TopischeTherapieMitHochkonzentriertemCapsaicinPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for topische-therapie
    if (lang === 'de') return '/topische-therapie-mit-hochkonzentriertem-capsaicin/';
    
    // All other languages use their language code in the URL
    return `/${lang}/topische-therapie-mit-hochkonzentriertem-capsaicin/`;
} 