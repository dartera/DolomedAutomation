...
// Define language configs for the Topische Therapie mit hochkonzentriertem Capsaicin page
const capsaicinLanguages = [
  {
  ...
  {
    url: '/fr/topische-therapie-mit-hochkonzentriertem-capsaicin/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Topische Therapie mit hochkonzentriertem Capsaicin Page (Desktop)', () => {
  ...
  for (const lang of capsaicinLanguages) {
    test(`capsaicin-visual-desktop-${lang.code}`, async () => {
...
test.describe('Visual Regression - Topische Therapie mit hochkonzentriertem Capsaicin Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of capsaicinLanguages) {
    test(`capsaicin-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'capsaicin-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
