...
// Define language configs for the Akupunktur page
const akupunkturLanguages = [
  {
    url: '/natuerliche-linderung-durch-akupunktur/',
    code: 'de',
    name: 'German (Default)',
    isDefault: true
  },
  {
    url: '/fr/natuerliche-linderung-durch-akupunktur/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Akupunktur Page (Desktop)', () => {
  ...
  for (const lang of akupunkturLanguages) {
    test(`akupunktur-visual-desktop-${lang.code}`, async () => {
...
test.describe('Visual Regression - Akupunktur Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of akupunkturLanguages) {
    test(`akupunktur-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'akupunktur-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
