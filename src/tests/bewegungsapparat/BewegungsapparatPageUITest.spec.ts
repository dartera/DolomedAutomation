...
// Define language configs for the Bewegungsapparat page
const bewegungsapparatLanguages = [
  {
...
test.describe('Visual Regression - Bewegungsapparat Page (Desktop)', () => {
  ...
  for (const lang of bewegungsapparatLanguages) {
    test(`bewegungsapparat-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'bewegungsapparat-visual',
        {
...
test.describe('Visual Regression - Bewegungsapparat Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of bewegungsapparatLanguages) {
    test(`bewegungsapparat-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'bewegungsapparat-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
