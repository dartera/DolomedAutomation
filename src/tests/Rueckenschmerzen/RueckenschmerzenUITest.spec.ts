    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `RueckenschmerzenPage_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `RueckenschmerzenPage_Mobile_${lang}`,
                    'RueckenschmerzenPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for rueckenschmerzen
    if (lang === 'de') return '/behandlung-von-rueckenschmerzen/';
    
    // All other languages use their language code in the URL
    return `/${lang}/behandlung-von-rueckenschmerzen/`;
} 