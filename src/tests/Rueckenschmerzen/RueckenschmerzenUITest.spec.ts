...
// Define language configs for the Rückenschmerzen page
const rueckenschmerzenLanguages = [
  {
    url: '/behandlung-von-rueckenschmerzen/',
    code: 'de',
    name: 'German (Default)',
    isDefault: true
  },
  {
    url: '/fr/behandlung-von-rueckenschmerzen/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Rückenschmerzen Page (Desktop)', () => {
  ...
  for (const lang of rueckenschmerzenLanguages) {
    test(`rueckenschmerzen-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'rueckenschmerzen-visual',
        {
...
test.describe('Visual Regression - Rückenschmerzen Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of rueckenschmerzenLanguages) {
    test(`rueckenschmerzen-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'rueckenschmerzen-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
