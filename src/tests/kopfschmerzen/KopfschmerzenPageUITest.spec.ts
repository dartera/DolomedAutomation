...
// Define language configs for the Kopfschmerzen page
const kopfschmerzenLanguages = [
    {
      url: '/kopfschmerzen-und-gesichtsschmerzen/',
      code: 'de',
      name: 'German (Default)',
      isDefault: true
    },
    {
      url: '/fr/kopfschmerzen-und-gesichtsschmerzen/',
      code: 'fr',
      name: 'French'
    }
  ];
...
test.describe('Visual Regression - Kopfschmerzen Page (Desktop)', () => {
  ...
  for (const lang of kopfschmerzenLanguages) {
    test(`kopfschmerzen-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'kopfschmerzen-visual',
        {
...
test.describe('Visual Regression - Kopfschmerzen Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of kopfschmerzenLanguages) {
    test(`kopfschmerzen-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'kopfschmerzen-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
