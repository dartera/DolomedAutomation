    ...
    for (const lang of languages) {
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `KopfschmerzenPage_Mobile_${lang}`,
                    'KopfschmerzenPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for kopfschmerzen
    if (lang === 'de') return '/kopfschmerzen-und-gesichtsschmerzen/';
    
    // All other languages use their language code in the URL
    return `/${lang}/kopfschmerzen-und-gesichtsschmerzen/`;
} 