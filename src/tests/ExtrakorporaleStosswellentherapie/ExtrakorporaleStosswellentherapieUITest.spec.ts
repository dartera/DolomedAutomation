...
// Define language configs for the Extrakorporale Stoßwellentherapie page
const stosswellentherapieLanguages = [
  {
  ...
  {
    url: '/fr/extrakorporale-stosswellentherapie/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Extrakorporale Stoßwellentherapie Page (Desktop)', () => {
  ...
  for (const lang of stosswellentherapieLanguages) {
    test(`stosswellentherapie-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'stosswellentherapie-visual',
        {
...
test.describe('Visual Regression - Extrakorporale Stoßwellentherapie Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of stosswellentherapieLanguages) {
    test(`stosswellentherapie-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'stosswellentherapie-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
