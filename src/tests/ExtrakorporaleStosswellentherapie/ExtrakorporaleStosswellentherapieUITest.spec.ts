    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ExtrakorporaleStosswellentherapie_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ExtrakorporaleStosswellentherapie_Mobile_${lang}`,
                    'ExtrakorporaleStosswellentherapiePage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for extrakorporale-stosswellentherapie
    if (lang === 'de') return '/extrakorporale-stosswellentherapie/';
    
    // All other languages use their language code in the URL
    return `/${lang}/extrakorporale-stosswellentherapie/`;
} 