    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `SchmerzenInnererOrganePage_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `SchmerzenInnererOrganePage_Mobile_${lang}`,
                    'SchmerzenInnererOrgane',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for schmerzen-innerer-organe
    if (lang === 'de') return '/schmerzen-innerer-organe/';
    
    // All other languages use their language code in the URL
    return `/${lang}/schmerzen-innerer-organe/`;
} 