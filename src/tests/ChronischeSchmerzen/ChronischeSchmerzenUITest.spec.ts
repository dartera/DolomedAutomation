...
// Define language configs for the Chronische Schmerzen page
const chronischeSchmerzenLanguages = [
  {
    url: '/behandlung-von-chronischen/',
    code: 'de',
    name: 'German (Default)',
    isDefault: true
  },
  {
    url: '/fr/behandlung-von-chronischen/',
    code: 'fr',
    name: 'French'
  }
];
...
test.describe('Visual Regression - Chronische Schmerzen Page (Desktop)', () => {
  ...
  for (const lang of chronischeSchmerzenLanguages) {
    test(`chronische-schmerzen-visual-desktop-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'chronische-schmerzen-visual',
        {
...
test.describe('Visual Regression - Chronische Schmerzen Page (Mobile)', () => {
  let visualHelper: MultiLanguageVisualHelper;
  ...
  for (const lang of chronischeSchmerzenLanguages) {
    test(`chronische-schmerzen-visual-mobile-${lang.code}`, async () => {
      await visualHelper.runIndividualLanguageTest(
        lang,
        'chronische-schmerzen-visual-mobile',
        {
          diffThreshold: visualTestDefaults.diffThreshold,
          customStyles: `* { max-width: 100% !important; overflow-x: hidden !important; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow-x: hidden !important; }`,
          scrollToBottom: visualTestDefaults.scrollToBottom,
