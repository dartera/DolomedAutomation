    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ChronischeSchmerzen_Full_${lang}`,
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `ChronischeSchmerzen_Mobile_${lang}`,
                    'ChronischeSchmerzenPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for chronische-schmerzen
    if (lang === 'de') return '/behandlung-von-chronischen/';
    
    // All other languages use their language code in the URL
    return `/${lang}/behandlung-von-chronischen/`;
} 