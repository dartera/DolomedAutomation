    ...
    for (const lang of languages) {
        test(`Desktop view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `NervenschmerzenPage_Full_${lang}`,
                    'NervenschmerzenPage',
                    test.info(),
                    comparisonOptions
                );
        ...
        test(`Mobile view - ${lang}`, async ({ browser }) => {
            ...
            try {
                ...
                const result = await comparePageScreenshot(
                    page,
                    `NervenschmerzenPage_Mobile_${lang}`,
                    'NervenschmerzenPage',
                    test.info(),
                    comparisonOptions
                );
...
function getLanguageUrl(lang: string): string {
    // German is served from the base path for nervenschmerzen
    if (lang === 'de') return '/behandlung-von-nervenschmerzen/';
    
    // All other languages use their language code in the URL
    return `/${lang}/behandlung-von-nervenschmerzen/`;
} 