...
export class TopischeTherapieMitHochkonzentriertemCapsaicinPage {
    protected page: Page;
    ...
    async verifyPageTitle() {
        ...
        expect(title).toContain('Effektive Schmerzlinderung durch moderne topische Therapie bei Dolomed');
    }

    async verifyMainContent() {
        const mainContent = await this.page.locator('p.text-18').first().textContent();
        expect(mainContent).toContain('Die topische Therapie mit hochkonzentriertem Capsaicin ist eine innovative Methode');
    }
