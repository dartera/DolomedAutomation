...
export class AkupunkturPage {
    ...
    // Hero Section
    async getHeroTitle() {
        const selector = '.elementor-element[data-id="5aeed2ad"] h1';
        await this.page.waitForSelector(selector, { state: 'visible' });
        return this.page.locator(selector).textContent();
    }

    async getHeroDescription() {
        const selector = '.elementor-element[data-id="d185432"] p';
        await this.page.waitForSelector(selector, { state: 'visible' });
        return this.page.locator(selector).textContent();
    }
    ...
    // FAQ Section
    async getFAQTitle() {
        const selector = '.elementor-element[data-id="600885b5"] h2';
        await this.page.waitForSelector(selector, { state: 'visible' });
        return this.page.locator(selector).textContent();
    }
    ...
    async getContactFormTitle() {
        const selector = '.elementor-element[data-id="3db79b11"] h2';
        await this.page.waitForSelector(selector, { state: 'visible' });
        return this.page.locator(selector).textContent();
    }
    ...
    async fillContactForm(data: {
        ...
        const formSection = '.elementor-element[data-id="3db79b11"]';
        ...
        const formSelector = '#wpcf7-f15505-p15907-o1';
    ...
    async getContactInfo() {
        ...
        const contactSection = '.elementor-element[data-id="641edd75"]';
        await this.page.locator(contactSection).scrollIntoViewIfNeeded();
        await this.page.waitForTimeout(300);

        return {
            biel: {
                address: await this.page.locator('.elementor-element[data-id="641edd75"] .elementor-icon-list-text').textContent(),
                phone: await this.page.locator('.elementor-element[data-id="444b05f0"] .elementor-icon-list-text').textContent(),
                email: await this.page.locator('.elementor-element[data-id="679e945a"] .elementor-icon-list-text').textContent()
            },
            bern: {
                address: await this.page.locator('.elementor-element[data-id="74bd919"] .elementor-icon-list-text').textContent(),
                phone: await this.page.locator('.elementor-element[data-id="3789662a"] .elementor-icon-list-text').textContent(),
                email: await this.page.locator('.elementor-element[data-id="5295697f"] .elementor-icon-list-text').textContent()
            }
        };
    }
} 